...
export const VideoBlock = (props:{video:VideoDetail,blockPerRow?:number,titleHeight?:number,backgroundColor?:string,descriptionPadding?:number,width?:number,titleSize?:number}):JSX.Element => {
	return(
    <div className={styles.video_block} style={{width:props.blockPerRow ? (100 / props.blockPerRow) + "%" : props.width}}>
      <Link href={`/video/${props.video.subcate_name.split("・").join("")}/${props.video.id}/${props.video.title}`}>
        <a className={styles.video_block_link}>
          ...
          <div className={styles.video_description} style={{backgroundColor:props.backgroundColor ? props.backgroundColor : "#f2f2f2",padding:props.descriptionPadding ? props.descriptionPadding : 3}}>
            <header className={styles.video_title} style={{height:props.titleHeight ? props.titleHeight : "fit-content",fontSize:props.titleSize ? props.titleSize : 14}}>{props.video.title}</header>
