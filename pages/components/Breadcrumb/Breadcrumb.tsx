    ...
    <div className={styles.breadcrumb}>
      {props.type === "video" && props.videos.filter((video) => video.id === (props.pid && props.pid[1])).map((video) =>
        <ReactBreadcrumb key={video.id}>
          ...
          <ReactBreadcrumb.Item href={`/category/${props.categories.filter((category) => category.cate_id === video.cate_id).map((category) => category.name_en.toLowerCase().split(" ").join("").split("/").join(""))}`} className={styles.breadcrumb_link}>
